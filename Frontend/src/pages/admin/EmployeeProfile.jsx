...
import { Bell, HelpCircle, Edit3, MoreVertical, Calendar, DollarSign, FileText, CheckCircle, XCircle, Clock, Plus, X } from 'lucide-react';
...
showToast(response.data.message);
  ...
  const attendanceData = [
    { month: 'January 2025', workingDays: 22, present: 21, absent: 1, onLeave: 2 },
    { month: 'December 2024', workingDays: 21, present: 20, absent: 0, onLeave: 1 },
    { month: 'November 2024', workingDays: 22, present: 22, absent: 0, onLeave: 0 },
  ];
            ...
            <div className="profile-card">
              ...
              <div className="profile-info">
                ...
                <div className="profile-meta">
                  <span className="job-title">📋 {capitalize(profile?.position)}</span>
                  <span className="emp-id">ID: {profile?.employeeId}</span>
                  <span className="status-badge">{capitalize(profile?.status)}</span>
                </div>
              </div>
                  ...
                  <div className="details-grid">
                    ...
                    <div className="detail-item">
                      ...
                      <div className="manager-card">
                        <div className="manager-avatar">{profile.reportingManager ? profile?.reportingManager.charAt(0).toUpperCase() : "NA"}</div>
                    ...
                    <div className="detail-item">
                      ...
                      <p className="detail-value">{new Date(profile.joiningDate).toLocaleDateString('en-GB', {
  day: 'numeric',
  month: 'long',
  year: 'numeric'
})}</p>
                    ...
                    <div className="detail-item">
                      ...
                      <p className="detail-value">{capitalize(profile?.jobType)}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="card">
                    ...
                    <div className="details-grid">
                      <div className="detail-item">
                        ...
                        <p className="detail-value"><i className="fa-solid fa-cake-candles"></i> - {profile?.dob || 'no record'}</p>
                      </div>

                      <div className="detail-item">
                        <label className="detail-label">Gender</label>
                        <p className="detail-value"><i className="fa-regular fa-user"></i> - {profile?.gender || "no record"}</p>
                      </div>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Total Working Days</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Total Present</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Total Absent</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">On Leave</th>
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Attendance %</th>
                      </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                      {attendanceData.map((record, idx) => {
                        const percentage = ((record.present / record.workingDays) * 100).toFixed(1);
                        return (
                          <tr key={idx}>
                            <td className="px-4 py-4 whitespace-nowrap">
                              <span className="font-semibold text-gray-900">{record.month}</span>
                            </td>
                            <td className="px-4 py-4 whitespace-nowrap text-gray-700">{record.workingDays}</td>
                            <td className="px-4 py-4 whitespace-nowrap">
                            ...
                            <td className="px-4 py-4 whitespace-nowrap">
                              <span className={`font-bold ${parseFloat(percentage) >= 95 ? 'text-green-600' : 'text-orange-600'}`}>
                                {percentage}%
                              </span>
                            </td>
                          </tr>
  ...
  <div className="space-y-4">
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm text-gray-600">{salaryData.length} records</span>
      </div>
    </div>
      ...
      <>
        ...
        <div className="md:hidden space-y-3">
          {salaryData.map((record, idx) => (
            <div key={record.employee || idx} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="font-bold text-gray-900">{record.month || 'N/A'} 2026</p>
                  <p className="text-lg font-bold text-blue-600 mt-1">₹{(record.netSalary || 0).toLocaleString()}</p>
                </div>
                <span
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">₹{(record.baseSalary || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-green-600">
                  <span>Allow:</span>
                  <span className="font-semibold">+₹{(record.allowances || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-red-600">
                  <span>Deduct:</span>
                  <span className="font-semibold">−₹{(record.deductions || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-amber-600">
                  <span>Tax:</span>
                  <span className="font-semibold">{record.taxApply || 0}%</span>
                </div>
              </div>
            </div>
          ))}
